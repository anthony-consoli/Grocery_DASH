...
public:
	...
	void setApplicationName(const wstring& initApplicationName)
	{
		applicationName = initApplicationName;
		//wstring name = std::string(initApplicationName.begin(), initApplicationName.end());
		SetWindowTextW(windowHandle, applicationName.c_str());
	...
	WindowsOS(	HINSTANCE hInstance,
				int nCmdShow,
				bool isFullscreen,
				const wstring& initApplicationName,
				int screenWidth,
				int screenHeight,
				Game *initGame);
	~WindowsOS();
