...
void OrthographicGridPathfinder::updatePath(AnimatedSprite *sprite)
{
	...
	b2Vec2 spriteP = sprite->body->GetPosition();
	float boundingX = spriteP.x  * 64.0f;
	float boundingY = spriteP.y  * 64.0f;
	...
	if ((currentPath->size() > 0) && (currentNode != currentPath->end()))
	{
		...
		{	
			...
			if (sprite->hasReachedDestination())
			{
				...
				sprite->body->SetLinearVelocity(b2Vec2_zero);
				sprite->clearPath();
				return;
			}
		}
		...
		float diffX = getColumnCenterX(node.column) - boundingX;
		float diffY = getRowCenterY(node.row) - boundingY;
		...
		pV.x = vX;
		pV.y = vY;
		sprite->body->ApplyForceToCenter(pV, true);

		sprite->setCurrentState(L"WALKING");
	}
	else
	{
		sprite->setCurrentState(L"STANDING");
	}	
}

bool OrthographicGridPathfinder::hasReachedNode(AnimatedSprite *sprite, PathNode destination)
{
	b2Vec2 spriteC = sprite->body->GetWorldCenter();
	float centerX = spriteC.x  * 64.0f;
	float centerY = spriteC.y  * 64.0f;

	//b2AABB spriteAABB = sprite->body->GetFixtureList()->GetAABB(0);
	//float xDiff = fabs(spriteAABB.GetCenter().x - getColumnCenterX(destination.column));
	//float yDiff = fabs(spriteAABB.GetCenter().y - getRowCenterY(destination.row));
	float xDiff = fabs(centerX - getColumnCenterX(destination.column));
	float yDiff = fabs(centerY - getRowCenterY(destination.row));
...
void OrthographicGridPathfinder::mapPath(AnimatedSprite *sprite, float worldX, float worldY)
{
	...
	buildPath(path, centerX, centerY, worldX, worldY);
